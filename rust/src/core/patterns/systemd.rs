...
fn compress_systemctl(cmd: &str, output: &str) -> String {
    ...
    if cmd.contains("list-units") || cmd.contains("list-unit-files") || (!cmd.contains("start") && !cmd.contains("stop") && !cmd.contains("restart") && !cmd.contains("enable") && !cmd.contains("disable")) {
...
fn compress_status(output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        if trimmed.starts_with("Active:") || trimmed.starts_with("Loaded:") ||
           trimmed.starts_with("Main PID:") || trimmed.starts_with("Memory:") ||
           trimmed.starts_with("CPU:") || trimmed.starts_with("Tasks:") {
...
fn compress_journal(output: &str) -> String {
    ...
    for line in &lines {
        ...
        let key = if parts.len() >= 4 { parts[3].to_string() } else { line.to_string() };
    ...
    let top: Vec<String> = sorted.iter().take(20).map(|(msg, count)| {
        if *count > 1 { format!("  ({count}x) {msg}") } else { format!("  {msg}") }
    }).collect();

    format!("{} log lines (deduped to {}):\n{}", lines.len(), top.len(), top.join("\n"))
}
