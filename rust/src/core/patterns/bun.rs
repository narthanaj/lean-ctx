...
fn compress_test(output: &str) -> String {
    ...
    for line in output.lines() {
        ...
        if (plain.contains("Ran") || plain.contains("Done")) && (plain.contains("ms") || plain.contains("s")) {
            time = plain.clone();
        }
    }
    ...
    if skipped > 0 { result.push_str(&format!(", {skipped} skipped")); }
...
fn compress_install(output: &str) -> String {
    ...
    if !time.is_empty() { parts.push(time); }
